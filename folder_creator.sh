...
directory=~/mf17/my_website/log/main_app
if [ ! -d $directory ]; then
  mkdir $directory
fi

# create log file for cron if needed 
CRON_FILE=~/mf17/my_website/log/main_app/main_app_cron.log
...
FILE=~/mf17/my_website/log/main_app/main_app.log
if test -f "$FILE"; then
   :
else
   touch $FILE
...
directory=~/mf17/my_website/vendor/engines/todoapp
if [ -d $directory ]; then
...
directory=~/mf17/my_website/vendor/engines/share_files_app
if [ -d $directory ]; then
